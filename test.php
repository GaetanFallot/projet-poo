...
require ('class/persoManager.class.php');
require ('class/profession.class.php');
require ('class/professionManager.class.php');
...
// le nombre de joueur inscris au contest
        public function getCountPlayerContest($id_contest){
            $requete = $this->_db->prepare('SELECT pc.id_player as id_player
            FROM player_contest pc
            LEFT JOIN player p
            ON p.id_player = pc.id_player
            LEFT JOIN contest c 
            ON c.id_contest = pc.id_contest
            WHERE c.id_contest = :id_contest');
            $requete->execute([
                'id_contest' => $id_contest,
            ]);
            while($donnees = $requete->fetch()){
                $player_contest = new Player_contest($donnees);
                $liste[] = $player_contest;
            }
            return @count($liste);
        }
    
        // liste de joueru à un contest
        public function getPlayerContest($id_contest){
            $liste = [];
            $requete = $this->_db->prepare('SELECT pc.id_player as id_player
            FROM player_contest pc
            LEFT JOIN player p
        ...
        // supprimer un joueur d'un contest
        public function deletePlayer($id_player, $id_contest){
            $requete = $this->_db->prepare('DELETE FROM player_contest
            WHERE id_player = :id_player AND id_contest = :id_contest');
            $requete->execute([
                'id_contest' => $id_contest,
                'id_player' => $id_player
            ]);
